...
example, under density-independent exponential population growth, the
...
%  \begin{gather}
%    \mu_t = N_{t-1}\exp(r) \label{eq:ss1a} \\ \label{eq:ss1b}
%    N_t \sim \text{Normal}(\mu_t, \grave{o}_t) \\ \label{eq:ss1c}
%    X_t \sim \text{Normal}(N_t, \zeta_t)
%  \end{gather}
%  \right\} \text{for}
%\end{subequations}
where $r$ is the intrinsic rate of increase. Note that no model is
presented for the initial state at $t=1$---this issue will be
discussed subsequently. Note also that the exponential
growth model shown in Eq.~\ref{eq:ss1a} can be easily replaced with
any population growth model, including density-depedent models. This
flexibility is an appealing feature of state-space models. One feature
that is common to classical state-space models is that the
error terms are assumed to be i.i.d random effects following Gaussian
