...
as:
%As an example, under
%density-independent exponential population growth, the
...
%    \mu_t &= N_{t-1}\exp(r) \quad \text{for} \; t=2,\hdots,T
  ...
  \end{align}
\end{subequations}
%{\flushleft where} $\mu(N_{t-1})$ is the deterministic population
%growth model. %One might alternatively consider density-dependent models
%for $\mu(N_{t-1})$, and this flexibility is an appealing feature of
%state-space models.
In classical state-space models, the two sets of random effects %In most classical state-space models both sets of parameters
