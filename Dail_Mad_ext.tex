...
population size \citep{jamieson_brooks:2004}.
...
%Originally developed for xxx (cite Kalman filter papers),
%The ability to model both the ecological state process and
...
%\citet{buckland_etal:2004} the definition : ``xxx'' .

One of the simplest state-space models can be described as follows.
% In their simplest form, state-space models include a Markovian model
%for the ecological process of interestTypically, the
%ecological
...
%Robservation process is typically phenomenological in the sense that
%observation error is not defined explicitly. As an example,
Let $N_t$ be the abundance of a species during year $t$, for
$t=1,\hdots,T$, and $X_t$ be
...
(e.g. exponential, logistic, etc \dots) and stochastic model allowing
for random process variation, which we denote $\grave{o}$ . As an
...
X_t = N_t + \zeta_t
\end{gather}
where $r$ is the intrinsic rate of increase. Typically, both random
