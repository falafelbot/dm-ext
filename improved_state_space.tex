...
%, which differs from $N_t$ due to observation error,
%a random effect denoted $\zeta_t$. 
...
observation error. 
%Often, parameter estimation requires fixing $\sigma_0$ at zero or assuming
%that initial abundance is an outcome of the theoretical stationary
%distribution of the first-order Markov chain \citep{devalpine_hastings:2002}.  
...
% that random effects are Gaussian distributed 
...
% assume that abundance can be take on any real value, and hence they
% can predict 
...
the $N$-mixture model uses replicate % original
observations at each site. The observations are made during sufficiently
short time intervals such that the population can be %safely
...
The first two processes describe the %the state process---
...
depend on $N_{i,t-1}$; %and which simulates a ``propagule rain'' of new individuals; 
...
%due to the exponential function relating to carrying capacity 
...
%population growth declines linearly with an increase in the natural logarithm of abundance,
%rather than linearly with an increase in abundance. 
...
detection probabilities for the BBS have contributed %from the large sample radii 
...
context: immigration conditional on total or mean % \citep{hanski:1998}
