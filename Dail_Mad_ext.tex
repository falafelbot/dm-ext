...
modeled using either the Poisson or negative binomial
distributions. Hence, in the Poisson case, we have
\begin{equation}
  [N_{i1} | \Lambda] = \frac{\Lambda^{N_{i1}}e^{-\Lambda}}{N_{i1}!}.
  \label{eq:N1br}
\end{equation}
where $\Lambda_i$ is the expected abundance at site $i$ during year 1.
...
\begin{equation}
  ...
  [n_{i1}|N_{i1}][N_{i1}|\Lambda] \left\{ \prod_{t=T}^2 \left\{ \sum_{N_{it}=0}^{\infty}
  [n_{it}|N_{it}][N_{it}|N_{it-1}] \right\} \right\} \right\}
  \label{eq:like}
\end{equation}
