...
%   \left.
%   \begin{aligned}
%     \mu_t &= N_{t-1}\exp(r) \\ %\label{eq:ss1b}
%     N_t &= \mu_t + \grave{o}_t \\ %\text{N}(\mu_t, \grave{o}_t) \\ %\label{eq:ss1c}
%     X_t &= N_t + \zeta_t %\text{N}(N_t, \zeta_t)
%   \end{aligned} \;
...
\begin{subequations}
  \begin{align}
%    \mu_t &= N_{t-1}\exp(r) \quad \text{for} \; t=2,\hdots,T
    N_1 &= X_1 \label{ss1a} \\
    N_t &= \mu_t + \grave{o}_{t-1} \quad \text{for} \; t=2,\hdots,T \label{eq:ss1b} \\
    X_t &= N_t + \zeta_t \quad\;\;\; \text{for} \; t=1,\hdots,T \label{eq:ss1c}
  \end{align}
  \label{eq:ss1}
\end{subequations}
where $\mu_t$ is a deterministic function describing population
growth. For instance, under exponential growth, $\mu_t =
N_{t-1}e^{r}$ where $r$ is the intrinsic rate of
increase. Alternatively, one might consider density-dependent models,
and.  This
flexibility is an appealing feature of state-space models.
Note that no model is
presented for the initial state at $t=1$; we will return to this issue
subsequently. Note also that the exponential
growth model shown in Eq.~\ref{eq:ss1} can be easily replaced with
any population growth model, including density-depedent models. One feature
that is common to most classical state-space models is that the
