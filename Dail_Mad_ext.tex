...
time-series data that attributes observation error to imperfect
detection.
We expand this class of models to accommodate classical growth
models (e.g. exponential, Ricker, etc $\dots$), zero-inflation, and
random effects such as observer-specific
...
\begin{comment}
  Irrespective of the model for population dynamics, it is worthwhile
  to note that the discrete distributions used in the DM model avoid
  the issues associated with the Gaussian distribution used to model
...
We also demonstrated how many of the objectives of classical
state-space models can be met using this. For example, one of the
primary aims of our paper was to demonstrate how classical
population growth models can be embedded in the DM model.
In some ways, such population growth models are more phenomenological
...
%assumption of geographic closure. Clearly,
estimating
...
%and are generally less supported in model selection.
%Adding
...
%data.
One strategy for adhereing to the explicit model would be to use
estimates of vital rates from demographic studies as priors in a
Bayesian analysis. Alternatively, count data could be combined with
other data types such as capture-recapture data, which offers a means of
making inferences about demographic processes at broad spatial
scales. Models that are fitted to count data and demographic data are
often referred to as integrated population models
\citep[][IPM]{besbeas_etal:2002, buckland_etal:2004,schaub_etal:2007}
and we believe that a DM model would provide an framework for an
IPM. However, we caution that when count data are affected by movement
and vital rates, IPMs would need to model
...
\begin{comment}
  The zero-inflated models for initial abundance and dynamics allow
  one to estimate and model the proportion of sites outside of the
  range of a species.  This could be especially useful when different
  factors control the range than the abundance or dynamics within the
  range.  [some examples where that is the case?]
\end{comment}
