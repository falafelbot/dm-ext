...
an extra dimension is added so tha twe have $n_{ijt}:
j=1,\hdots,J$. Typically, we $n_{it}$ will be lower than the actual
quantitiy of interest, abundance,
denoted $N_{it}$. In cases where detection probability is perfect,
abundance is observed directly such that $n_{it} \equiv N_{it}$.
...
1. In the Poisson case, the eqivalent expression of
Eq.~\ref{eq:ss2a} is $[N_{i1} | \Lambda] =
\frac{\Lambda^{N_{i1}}e^{-\Lambda}}{N_{i1}!}$.
%From hereout, we will reserve bracket notation for arbitrary
%probability distributions, and for simplicity, use the equivalent notation $N_{i1}
%\sim \text{Pois}(\Lambda_i)$ when referencing specific distributions.
The Poisson distribution assumes that the mean of $N_{i1}$ is equal to its
variance, whereas the negative binomial distribution, $N_{i1} \sim
\text{NB}(\Lambda, \alpha)$, allows the
variance to be greater than the mean with the amount of overdispersion
determined by the parameter $\alpha$.
Note that $N_{i1} \sim \text{Pois}(\Lambda)$ is equivalent to
$[N_{i1} | ] = \frac{\Lambda^{N_{i1}}e^{-\Lambda}}{N_{i1}!}$.
...
$\gamma$ is the sum %\hl{[I think!]}
of the birth rate and the immigration rate,
i.e. the recruitment rate. In the former case where no movement
occurs, the DM model can directly estimate
...
the likelihood tractible, the general expression of which can be written:
\begin{equation}
  \mathcal{L}(\Lambda, \Phi, \mathbf{p} | \{n_{it}\}) =
  \prod_{i=1}^R \left\{ \sum_{N_{i1}=0}^{\infty}
  [n_{i1}|N_{i1}][N_{i1}|\Lambda] \left\{
%  \prod_{t=T}^2 \left\{
  \sum_{N_{i1}=0}^{\infty} \dotsm \sum_{N_{iT}=0}^\infty
  [n_{it}|N_{it}][N_{it}|N_{it-1}] %\right\}
  \right\} \right\}
  \label{eq:like}
\end{equation}
This likelihood can be maximized numerically using the \textbf{R}
...
here is \textbf{BUGS} code for the ``constant'' model described above.
